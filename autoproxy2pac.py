...
with closing(urllib.urlopen(gfwlistUrl)) as response:
    ...
    gfwlistDate = response.info().getheader('last-modified')

#===============================================================================
# Generate/Update PAC file
#===============================================================================
...
try:
    ...
    parsed = re.match(r'''(?: %s)?               # description text
                          (.+ ^ \s*? %s \s*?)    # header
                          ^.*                    # auto-generated rules
                          (^ \s*? %s \s*? ^ .+)  # footer
                       ''' % (re.escape(descTemplate).replace('\\%s', '.*'),
                              re.escape(rulesBegin),
                              re.escape(rulesEnd)),
                      content,
                      re.MULTILINE | re.DOTALL | re.VERBOSE)
    
    if parsed:
        pacHeader, pacFooter = parsed.groups()
    else:
        print("WARNING: Can not find auto-generated rule section, user-defined rules will LOST during the update")
...
with open(pacFilepath, 'w') as f:
    f.write(descTemplate % (gfwlistUrl, gfwlistDate))
    f.write(pacHeader)
